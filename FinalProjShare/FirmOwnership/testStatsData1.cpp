...
int main() {
	...
	int totPop = 0;
	for (auto entry : theCounties.countyDmap()) {
		double y=entry.second->getED().getIndian();
		double x=entry.second->getED().getAsian();
		if(isnan(y)){
			dataYcount.push_back(0);
		}else{
			dataYcount.push_back(y);
		}
		if(isnan(x)){
			dataXcount.push_back(0);
		}
		else{
			dataXcount.push_back(x);
		}
		//totPop += entry.second->getPop();
		/*dataYpercent.push_back(entry.second->getBelowPoverty());
    dataXpercent.push_back(entry.second->getHSup());*/
	}
	double ans=stats::computeCorCoeff(dataYcount,dataXcount);
	/*
  ...
  ASSERT_EQUALS(stdDevY, 3.177);
	 */
	cout<<"Corellation Coefficient: "<<ans<<endl;
	return 0;
}
