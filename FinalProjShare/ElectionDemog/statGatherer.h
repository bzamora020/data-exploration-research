...
class statGatherer {
  public:
    ...
    virtual void gatherPerStats(Visitor* theCounties, vector<double> &XPer, vector<double> &YPer, 
        double (demogData::*f1)() const, double (comboElectionData::*f2)() const) = 0;
};
...
class countyGather : public statGatherer {
  public:
    ...
    //for all demographic data
        for (auto entry : ((visitorCombineCounty *)theCounties)->countyDmap()) {
            //make sure there is valid hospital data!
            comboElectionData* elecForCounty = ((visitorCombineCounty *)theCounties)->countyEmapEntry(entry.first);
            if ( elecForCounty != NULL ) {
                double xP = (entry.second->*f1)();
                double yP = (elecForCounty->*f2)();
                if (!isnan(xP) && !isnan(yP)) {
                    YPer.push_back(yP);
                    XPer.push_back(xP);
                }
...
class stateGather : public statGatherer {
  public:
    ...
    void gatherPerStats(Visitor* theCounties, vector<double> &XPer, vector<double> &YPer, 
        double (demogData::*f1)() const, double (comboElectionData::*f2)() const){ return;}
};
#endif