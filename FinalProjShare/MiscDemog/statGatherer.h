...
class statGatherer {
  public:
    ...
    virtual void gatherPerStats(const Visitor* Regions, vector<double> &XPer, vector<double> &YPer, 
    ...
    //                                 int (hospitalData::*f3)() const, int (hospitalData::*f4)() const) = 0;
...
class countyGather : public statGatherer {
  public:
    ...
    //function to gather mixed data (demographic and hospital)
    void gatherPerStats(const Visitor* theCounties , vector<double> &XPer, vector<double> &YPer, 
            double (demogData::*f1)() const, double (comboHospitalData::*f2)() const) {
        //for all demographic data
        //cout << endl << ((visitorCombineCounty*)theCounties)->countyHmap().size() << "here1" << endl;
        for (auto entry : ((visitorCombineCounty *)theCounties)->countyDmap()) {
            ...
            if ( hospForCounty != NULL ) {
                //cout << ((visitorCombineCounty*)theCounties)->countyHmap().size()  << "hereloop" << endl;
        ...
        //cout << endl << ((visitorCombineCounty*)theCounties)->countyHmap().size() << "here3" << endl;
    }
    //function to gather two demog datas
...
class stateGather : public statGatherer {
  public:
    ...
    //function to gather mixed data (demographic and hospital)
    void gatherPerStats(const Visitor* theStates, vector<double> &XPer, vector<double> &YPer, 
            double (demogData::*f1)() const, double(comboHospitalData::*f2)() const) {
