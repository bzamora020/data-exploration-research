...
/* compute statistics for hospital data
expects, the region, a statGatherer and function pointers for the methods to fill in */
void statTool::computeStatsHospitalRegionData(Visitor* theRegions, statGatherer* fillArrays, 
...
void statTool::computeStatsDemogHouseRegionData(Visitor* theRegions, statGatherer* fillArrays, 
    ...
    //fill in the datausing a statGatherer
    fillArrays->gatherPerStats(theRegions, dataX, 
        dataY, f1, f2);

    double mX = stats::computeMean(dataX); 
   
    double mY = stats::computeMean(dataY);  
    cout << "REGION stats comparing demographic and housing data " << endl;
    cout << "stats mean X: " << mX    << " size of vector: " << dataX.size() << endl;
    cout << "stats mean Y: " << mY << " size of vector: " << dataY.size() << endl;
