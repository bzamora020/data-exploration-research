...
// Function to create a report using poverty level as tresh hold

void dataAQ::stateReport(double thresh)
{
    int counter = 0;
    visitorReport printReport;
            cout <<endl;
    for (auto it : allStateDemogData)
    {
        if (it.second->getBelowPoverty() > thresh)
        {
            it.second->accept(printReport);
            allStateHospData[it.second->getState()]->accept(printReport);
            counter++;
        }
    }
    cout << "Generated a report for a total of: " << counter << endl;
}
