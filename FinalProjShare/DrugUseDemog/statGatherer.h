...
class statGatherer {
  public:
    ...
    //function to gather mixed data (demographic and drug)
    virtual void gatherPerStats(Visitor* theStates, vector<double> &XPer, vector<double> &YPer, 
            double (demogData::*f1)() const, double(drugData::*f2)() const){ cout << "demographic and drug didn't override" << endl;}

     //function to gather two drug datas
    virtual void gatherPerStats(Visitor* theStates, vector<double> &XPer, vector<double> &YPer, 
            double (drugData::*f1)() const, double(drugData::*f2)() const){cout << "2 drug datas didn't override" << endl;}


    //function to gather mixed data (drug and hospital)
    virtual void gatherPerStats(Visitor* theStates, vector<double> &XPer, vector<double> &YPer, 
            double (drugData::*f1)() const, double(comboHospitalData::*f2)() const){cout << "drug and hospital didn't override" << endl;}
    ...
    //function to gather two drug datas (percentages and counts)
    virtual int gatherBothStats(Visitor* theStates, vector<double> &XPer, vector<double> &YPer,
                                    vector<double> &XCount, vector<double> &Ycount,
                                    double (drugData::*f1)() const, double(drugData::*f2)() const,
                                    int (drugData::*f3)() const, int (drugData::*f4)() const) {cout << "drug and drug mixed didn't override" << endl;
...
class stateGather : public statGatherer {
  public:
    ...
    void gatherPerStats(Visitor* theStates, vector<double> &XPer, vector<double> &YPer, 
            double (drugData::*f1)() const, double(drugData::*f2)() const) {
        for (auto entry : ((visitorCombineState *)theStates)->stateDrugmap()) {
            XPer.push_back((entry.second->*f1)());
            YPer.push_back((entry.second->*f2)());
        }
    }
    ...
    //function to gather mixed data (demographic and drug)
    void gatherPerStats(Visitor* theStates, vector<double> &XPer, vector<double> &YPer, 
            double (demogData::*f1)() const, double(drugData::*f2)() const) {
        for (auto entry : ((visitorCombineState *)theStates)->stateDmap()) {
            drugData* drugForState = ((visitorCombineState *)theStates)->stateDrugmapEntry(entry.first);
            XPer.push_back(((entry.second)->*f1)());
            YPer.push_back((drugForState->*f2)());
        }
    }
    ...
    //function to gather mixed data (drug and hospital)
    void gatherPerStats(Visitor* theStates, vector<double> &XPer, vector<double> &YPer, 
            double (drugData::*f1)() const, double(comboHospitalData::*f2)() const) {
        for (auto entry : ((visitorCombineState *)theStates)->stateDrugmap()) {
          comboHospitalData* hospForState = ((visitorCombineState *)theStates)->stateHmapEntry(entry.first);
            if (hospForState  != NULL) {
                XPer.push_back(((entry.second)->*f1)());
    ...
    //                                 vector<double> &XCount, vector<double> &Ycount,
    //                                 double (demogData::*f1)() const, double(drugData::*f2)() const,
    //                                 int (demogData::*f3)() const, int (drugData::*f4)() const) {
    //     //fill in the percentage stats
    //     gatherPerStats(theStates, XPer, YPer, f1, f2);
    ...
    //         drugData* drugForState = ((visitorCombineState *)theStates)->stateDrugmapEntry(entry.first);
    //         XCount.push_back((entry.second->*f3)());
    //         Ycount.push_back((drugForState->*f4)()); 
    //         totPop += entry.second->getPop(); 
    //     }
    //     return totPop;
    // }
    ...
    int gatherBothStats(Visitor* theStates, vector<double> &XPer, vector<double> &YPer,
                                    vector<double> &XCount, vector<double> &Ycount,
                                    double (drugData::*f1)() const, double(drugData::*f2)() const,
                                    int (drugData::*f3)() const, int (drugData::*f4)() const) {
        //fill in the percentage stats
        gatherPerStats(theStates, XPer, YPer, f1, f2);
        ...
        for (auto entry : ((visitorCombineState *)theStates)->stateDrugmap()) {
            XCount.push_back((entry.second->*f3)());
            Ycount.push_back((entry.second->*f4)());
            totPop += entry.second->getpop18to25(); 
        }
        return totPop;
    }
